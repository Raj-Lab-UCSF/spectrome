...
def eig_fc_get_standardz(x,y, binary_thresh = 0.1, binarize = False, nreps = 1000):
...
def get_overlap_score_dfs(all_eig, all_fc_networks, binarize = False, threshold = 0.24):
    """[summary]
    ...
    for eignum in df_sxy.index:
        for name in all_fc_networks.index:
            ...
            df_overlap_score.at[eignum, name], df_sxy.at[eignum, name] = eig_fc_get_standardz(eig_vector, fc_vector, binary_thresh = threshold, binarize = binarize)
        eigcounter += 1
