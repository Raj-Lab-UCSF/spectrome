...
parser.add_argument(
    "--tau_e", type=float, default=0.012, help="Excitatory time constant"
)
parser.add_argument(
    "--tau_i", type=float, default=0.003, help="Inhibitory time constant"
)
...
hbp = firls(
    101,
    np.array([0, 0.2 * fmin, 0.9 * fmin, fmax - 2, fmax + 5, 100]) * 2 / fs,
        ...
        for gii_iter in gii:
            for tauc_iter in tauC:
                ...
                file_name = make_filename(
                    tau_e, tau_i, alpha, speediter, gei_iter, gii_iter, tauc_iter
                )

                freq_model = []
                for i in fvec:
                    w = 2 * np.pi * i
                    fq, ev, Vv, freqresp_out, _ = nt.network_transfer_function(
                        mybrain, parameters=mybrain.ntf_params, w=2 * np.pi
                    )
